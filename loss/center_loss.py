...
class CenterLoss(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        distmat.addmm_(1, -2, x, self.centers.t())   #distmat=[16,751]

        classes = torch.arange(self.num_classes).long()  #[751]
        if self.use_gpu: classes = classes.cuda()
        labels = labels.unsqueeze(1).expand(batch_size, self.num_classes)
        mask = labels.eq(classes.expand(batch_size, self.num_classes))  #[16,751]
